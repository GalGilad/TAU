...
def find_partition():
    ...
    for generation_i in range(1, MAX_GENERATIONS+1):
        ...
        s1_time = time.time()
        if VERBOSE:
            print(f'Stage one (optimization) time: {s1_time - start_time}')
        ...
        s2_time = time.time()
        if VERBOSE:
            print(f'Stage two (stopping, elitist selection) time: {s2_time - s1_time}')
        ...
        s3_time = time.time()
        if VERBOSE:
            print(f'Stage Three (crossover, immigration) time: {s3_time - s2_time}')
        ...
        s4_time = time.time()
        if VERBOSE:
            print(f'Stage Four (mutation) time: {s4_time - s3_time}')
